...
function calculateCombatRating(stats) {
    ...
    let score = (atk * 1) + (def * 2.5) + (cr * 20) + (cd * 5) + (dr * 25) + (ci * 10);
...
async function addUnitToDB(name, job) {
    ...
    await addDoc(collection(db, "users", currentUser.uid, "units"), {
        ...
        team: 'res',
        status: 'ON_CAMP',
        stats: { atk: 0, def: 0, crit_rate: 0, crit_dmg: 0, dmg_red: 0, crit_res: 0 },
        createdAt: Date.now()
    });
}
...
document.getElementById('btn-save-detail').addEventListener('click', async () => {
    ...
    await updateDoc(unitRef, { name: newName, status: newStatus, stats: newStats });
...
function renderGlobalLeaderboard() {
    ...
    globalCampsData.forEach((camp, index) => {
        ...
        el.innerHTML = `
            ...
            <div class="lb-power"><i class="fa-solid fa-bolt"></i> ${camp.totalPower.toLocaleString()}</div>
        `;
        ...
        <div class="war-detail-card active">
            <div style="display:flex; justify-content:space-between; align-items:flex-end; border-bottom:1px solid var(--border); padding-bottom:15px;">
                <div>
                    ...
                    <div style="font-size:0.9rem; color:${rank < 3 ? 'var(--warn)' : 'var(--text-muted)'}; font-weight:600;">GLOBAL RANKING: #${rank + 1}</div>
                </div>
                <div style="text-align:right;">
                    <div style="font-size:3rem; font-weight:800; color:var(--accent); line-height:1;">${(camp.totalPower / 1000).toFixed(1)}<span style="font-size:1.5rem">k</span></div>
            ...
            <div style="flex:1; display:flex; flex-direction:column; justify-content:center;">
                <div style="font-size:0.85rem; text-transform:uppercase; letter-spacing:1px; margin-bottom:15px; color:var(--text-muted); text-align:center;">Market Comparison (Vs Global Top)</div>
...
function renderList() {
    ...
    processedData.sort((a, b) => {
        ...
        switch(currentSort.key) {
            ...
            case 'job': valA = a.job.toLowerCase(); valB = b.job.toLowerCase(); break;
            case 'rating': valA = calculateCombatRating(a.stats); valB = calculateCombatRating(b.stats); break;
            case 'team': valA = TEAMS.findIndex(t => t.id === a.team); valB = TEAMS.findIndex(t => t.id === b.team); break;
            case 'class': valA = getJobInfo(a.job).cat; valB = getJobInfo(b.job).cat; break;
    ...
    processedData.forEach(p => {
        ...
        tr.innerHTML = `
            ...
            <td style="text-align:right; white-space:nowrap;">
                <button class="btn-ghost" style="padding:6px 10px; color:var(--warn); border:none; margin-right:5px;" onclick="window.openCardGenerator('${p.id}')" title="Generate ID Card">
...
function runComparison() {
    ...
    const s2 = p2.stats || {};
    const statsConfig = [
        { key: 'atk', label: 'Attack Power', max: 315, type: 'offense' },
        { key: 'crit_rate', label: 'Crit Rate %', max: 350, type: 'offense' },
        { key: 'crit_dmg', label: 'Crit Dmg %', max: 350, type: 'offense' },
        { key: 'def', label: 'Armor / Def', max: 290, type: 'defense' },
        { key: 'dmg_red', label: 'Dmg Reduct %', max: 350, type: 'defense' },
        { key: 'crit_res', label: 'Crit Immun %', max: 350, type: 'defense' }
    ];
...
function updateRadarChart(p1, p2, s1, s2) {
    ...
    const data1 = [
        norm(s1.atk, 315), norm(s1.crit_rate, 350), norm(s1.crit_dmg, 350), 
        norm(s1.def, 290), norm(s1.dmg_red, 350), norm(s1.crit_res, 350)
    ];
    const data2 = [
        norm(s2.atk, 315), norm(s2.crit_rate, 350), norm(s2.crit_dmg, 350), 
        norm(s2.def, 290), norm(s2.dmg_red, 350), norm(s2.crit_res, 350)
    ];
...
window.openCardGenerator = (id) => {
    const p = players.find(u => u.id === id);
    if(!p) return;
    const score = calculateCombatRating(p.stats);
    const s = p.stats || {};
    let rank = "F", title = "ROOKIE", skin = "skin-iron", tierName = "IRON TIER";
    if(score >= 2500) { rank="E"; title="SCOUT"; skin="skin-iron"; tierName="IRON TIER"; }
    if(score >= 5000) { rank="D"; title="VETERAN"; skin="skin-iron"; tierName="STEEL TIER"; }
    if(score >= 7500) { rank="C"; title="ELITE"; skin="skin-gold"; tierName="GOLD TIER"; }
    if(score >= 10000) { rank="B"; title="COMMANDER"; skin="skin-gold"; tierName="GOLD TIER"; }
    if(score >= 14000) { rank="A"; title="ROYAL GUARD"; skin="skin-platinum"; tierName="PLATINUM TIER"; }
    if(score >= 17500) { rank="S"; title="HERO"; skin="skin-platinum"; tierName="PLATINUM TIER"; }
    if(score >= 20000) { rank="SS"; title="DEMIGOD"; skin="skin-mythic"; tierName="MYTHIC TIER"; }
    if(score >= 21500) { rank="SSS"; title="LEGEND"; skin="skin-mythic"; tierName="MYTHIC TIER"; }
    document.getElementById('card-name').innerText = p.name;
    document.getElementById('card-job').innerText = p.job;
    document.getElementById('card-team').innerText = TEAMS.find(t=>t.id===p.team)?.label || 'Unknown';
    document.getElementById('card-uid').innerText = p.id.substr(0,8).toUpperCase();
    document.getElementById('card-score').innerText = score.toLocaleString();
    document.getElementById('card-rank').innerText = rank;
    document.getElementById('card-title').innerText = title;
    const cardEl = document.getElementById('adventurer-card');
    cardEl.className = ""; 
    cardEl.classList.add(skin);
    document.getElementById('info-tier').innerText = tierName;
    document.getElementById('info-tier').style.color = (skin === 'skin-mythic') ? '#d946ef' : (skin === 'skin-gold' ? '#facc15' : 'white');
    const pctAtk = Math.min((parseInt(s.atk||0) / 315) * 100, 100);
    const pctDef = Math.min((parseInt(s.def||0) / 290) * 100, 100);
    const pctCrit = Math.min((parseInt(s.crit_rate||0) / 350) * 100, 100);
    document.getElementById('bar-atk').style.width = `${pctAtk}%`;
    document.getElementById('bar-atk').style.backgroundColor = '#ef4444'; 
    document.getElementById('bar-def').style.width = `${pctDef}%`;
    document.getElementById('bar-def').style.backgroundColor = '#3b82f6'; 
    document.getElementById('bar-crit').style.width = `${pctCrit}%`;
    document.getElementById('bar-crit').style.backgroundColor = '#facc15'; 
    
    // Default Avatar logic (bisa ditimpa upload nanti)
    const initialName = p.name.replace(/\s+/g, '+');
    document.getElementById('card-img').src = `https://ui-avatars.com/api/?name=${initialName}&background=1e293b&color=fff&size=128`;
    
    document.getElementById('modal-card-gen').style.display = 'flex';
};

document.getElementById('btn-close-card').addEventListener('click', () => {
    document.getElementById('modal-card-gen').style.display = 'none';
});

// --- FITUR UPLOAD FOTO & DOWNLOAD DIPERBARUI DI SINI ---

document.getElementById('card-img-upload').addEventListener('change', function(e) {
    if(e.target.files[0]) {
        const reader = new FileReader();
        reader.onload = (ev) => document.getElementById('card-img').src = ev.target.result;
...
document.getElementById('btn-download-card').addEventListener('click', () => {
    ...
    const card = document.getElementById('adventurer-card');
    ...
    html2canvas(card, {
        ...
        useCORS: true,       // PENTING: Izinkan gambar lintas origin/lokal
        allowTaint: false    // Hindari taint canvas
    }).then(canvas => {
        const link = document.createElement('a');
        const unitName = document.getElementById('card-name').innerText;
        // Format nama file agar lebih rapi
        link.download = `ID_Card_${unitName.replace(/\s+/g, '_')}.jpg`;
        link.href = canvas.toDataURL('image/jpeg', 0.9);
        link.click();
        
        btn.innerHTML = '<i class="fa-solid fa-download"></i> Download JPG';
        btn.disabled = false;
    }).catch(err => {
        console.error(err);
        alert("Gagal generate gambar. Cek console.");
        btn.innerHTML = '<i class="fa-solid fa-download"></i> Download JPG';
        btn.disabled = false;
    });
});
